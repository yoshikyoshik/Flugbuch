export default async function handler(request, context) {
    ...
    try {
        ...
        const cleanFlights = uniqueFlights.map(f => {
            // 🚀 BUGHUNT FIX: Namen & Städte sauber extrahieren!
            let orig = "N/A";
            let origName = "";
            if (f.origin && f.origin !== "null") {
                if (typeof f.origin === 'object') {
                    orig = f.origin.code_iata || f.origin.code_icao || "N/A";
                    origName = f.origin.city || f.origin.name || "";
                } else {
                    orig = f.origin;
                }
            }
            ...
            if (f.destination && f.destination !== "null") {
                if (typeof f.destination === 'object') {
                    dest = f.destination.code_iata || f.destination.code_icao || "N/A";
                    destName = f.destination.city || f.destination.name || "";
                } else {
                    dest = f.destination;
                }
            }
            ...
            return {
                ...
                origin: orig,
                origin_name: origName,             // NEU
                destination: dest,
                destination_name: destName,        // NEU
